...
| `HERMES_STREAM_STALE_GIVEUP` | Cross-turn circuit breaker: after this many consecutive stale kills (streaming or non-streaming) with no completed response, abort each call immediately with an actionable error instead of re-waiting out the stale timeout (default: `5`, `0` disables). After `agent.stale_breaker_cooldown_seconds` (default `300`, `0` = permanent latch) the breaker half-opens and allows one real probe; success resets the streak, failure re-opens. Also resets on any completed response, `/model` switch, fallback activation, or turn-start primary restore. |
